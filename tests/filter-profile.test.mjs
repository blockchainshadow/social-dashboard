...
for (const page of ["../index.html", "../web/index.html"]) test(`${page}: filtering keeps profile in sync`, () => {
  ...
  const dom = new JSDOM(`<select id="group-filter"><option value="__all__">全部分组</option><option value="公司">公司</option></select><select id="tag-filter"><option value="__all__">全部标签</option></select><select id="channel"><option value="youtube:mkbhd">MKBHD</option><option value="youtube:game">[YT] 矩阵游戏1</option></select>`);
  ...
  const context = {
    ...
    channels: {
      "youtube:mkbhd": { info: { group: "科技", tags: [] } },
      "youtube:game": { info: { group: "公司", tags: [] } },
    },
    authAllows: () => true,
